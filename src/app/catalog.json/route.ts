...
export async function GET() {
  ...
  const personasWithStars = personas.map((p) => ({
    ...p,
    stars: stars[p.slug] ?? null,
  }));
  ...
  const catalog = {
    ...
    description:
      "personas.sh persona catalog. AI agents: fetch this URL to search all available personas.",
    search_tips:
